import {
  ...
  class ButtonCard extends LitElement {
    ...
    icon(state, config) {
      ...
      const color_off = state.attributes.color_off ? `rgb(${state.attributes.rgb_color.join(',')})` : (
        config.color_off ? config.color_off : "var(--disabled-text-color)"
      )
      const color_on = state.state === 'on' ? color : color_off;
      let card_style = '';
      if (config.style){
